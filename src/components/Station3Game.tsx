...
import { Lightbulb, Sun, Cloud, CloudOff, Wind, BatteryCharging, Volume2, VolumeX } from 'lucide-react';
...
const playSound = (soundName: string, isMuted: boolean) => {
  ...
  audio.volume = 0.5;
  if (soundName === 'click') audio.volume = 0.3;
...
function DraggablePanel({ isMuted }: { isMuted: boolean }) {
...
function GameContent({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    let voiceFile = '';
    switch (guideMessage) {
      case "¡Modernicemos la ciudad! Usa energía solar. ☀️":
        voiceFile = 'voice_city_intro';
        break;
      case "¡Bien! Apagando luces innecesarias. 💡":
        voiceFile = 'voice_city_lights_off';
        break;
      case "¡Panel instalado! Energía limpia generada. ⚡":
        voiceFile = 'voice_city_panel_placed';
        break;
      case "¡Ciudad Futura completada! 🚀":
        voiceFile = 'voice_city_complete';
        break;
    }
      ...
      <div className="relative z-30 flex-grow w-full max-w-6xl mx-auto mt-4 perspective-[1200px] overflow-hidden">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg p-4 rounded-t-[2rem] shadow-[0_-10px_50px_rgba(0,0,0,0.2)] flex justify-between items-center max-w-4xl mx-auto border-t-4 border-blue-400 z-50 pb-8 md:pb-4">
           ...
           <div className="flex items-center gap-4 pl-4">
              ...
              <DraggablePanel isMuted={isMuted} />
           </div>
