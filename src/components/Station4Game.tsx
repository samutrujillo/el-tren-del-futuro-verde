...
import { Cloud, Heart, Mountain, Volume2, VolumeX } from 'lucide-react';
...
function DraggableAnimal({ animal, isMuted }: { animal: Animal, isMuted: boolean }) {
  ...
  useEffect(() => {
    if (isDragging) playSound('pop', isMuted);
  }, [isDragging, isMuted]);
...
function HabitatZone({ type, onDrop, animals }: HabitatProps) {
  ...
  if (type === 'sky') {
     return (
        <>
           {/* Logic Drop Zone: Invisible, large, top layer, NO 3D TRANSFORMS */}
           <div 
              ref={drop}
              className="absolute top-0 left-0 w-full h-1/2 z-20"
              style={{ transform: 'none' }} // Force flat rendering for hit testing
           />
           
           {/* Visuals: 3D Deep Layer */}
           <div className="w-full h-full flex items-center justify-center transform-style-3d pointer-events-none">
               {isOver && (
  ...
  return (
    <div 
      ref={drop}
      className="relative transition-all duration-500 perspective-[1000px] w-1/2 h-[300px] md:h-[400px] flex flex-col items-center justify-end"
    >
...
function GameContent({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    let voiceFile = '';
    switch (guideMessage) {
      case "¡Rescata a los animales! Arrástralos a su hogar. 🐾":
        voiceFile = 'voice_mountain_intro';
        break;
      case "¡Bien hecho! ¡Libre al fin! ❤️":
        voiceFile = 'voice_mountain_rescued';
        break;
      case "¡Fauna protegida! La montaña sonríe. 🏔️":
        voiceFile = 'voice_mountain_complete';
        break;
    }
    ...
    <div className="w-full h-full relative overflow-hidden font-sans flex flex-col select-none">
      ...
      <div className="relative z-40 p-2 md:p-4 flex flex-col items-center shrink-0">
        <div className="w-full flex justify-between items-start max-w-6xl">
           ...
           <Button 
             ...
             className="bg-white/50 hover:bg-white/80 rounded-full text-emerald-900"
           >
      ...
      <div className="relative z-30 flex-grow w-full max-w-6xl mx-auto flex flex-col justify-between pb-4 perspective-[1200px] overflow-hidden">
        ...
        <div className="relative flex-grow w-full transform-style-3d flex flex-col justify-end pb-20 md:pb-0">
           {/* Sky Habitat - Now using cleaner rendering approach in component */}
           ...
           {!allRescued && (
             <div className="bg-gray-900/60 backdrop-blur-md p-2 md:p-4 rounded-3xl border-t-4 border-gray-600 mx-4 max-w-3xl md:mx-auto">
               ...
               <div className="flex flex-wrap justify-center gap-2 md:gap-4 min-h-[80px] md:min-h-[100px]">
                 {animals.map(animal => (
                   <DraggableAnimal key={animal.id} animal={animal} isMuted={isMuted} />
                 ))}
               </div>
             </div>
           )}
