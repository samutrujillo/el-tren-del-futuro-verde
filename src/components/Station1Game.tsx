...
import { Sprout, Trees, Droplets, CloudSun, Volume2, VolumeX } from 'lucide-react';
...
const playSound = (soundName: string, isMuted: boolean) => {
  ...
  audio.volume = 0.5;
  if (soundName === 'success') audio.volume = 0.4;
...
function Tool({ type, isMuted }: { type: 'seed' | 'water', isMuted: boolean }) {
...
function GameContent({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    let voiceFile = '';
    switch (guideMessage) {
      case "¡Bienvenido al Bosque! Arrastra las semillas a la tierra. 🌱":
        voiceFile = 'voice_forest_intro';
        break;
      case "¡Bien sembrado! Ahora necesita agua. 💧":
        voiceFile = 'voice_forest_seeded';
        break;
      case "¡Creció un árbol! ¡El bosque revive! 🌳":
        voiceFile = 'voice_forest_grown';
        break;
      case "¡Primero necesitas una semilla! 🌱":
        voiceFile = 'voice_forest_need_seed';
        break;
      case "¡Este árbol ya está fuerte y sano! ✨":
        voiceFile = 'voice_forest_already_grown';
        break;
      case "¡Misión Cumplida! Mira cómo regresan los animales. 🦋🐦":
        voiceFile = 'voice_forest_complete';
        break;
    }
    ...
    <div className="w-full h-full relative overflow-hidden font-sans flex flex-col select-none">
      ...
      <div className="relative z-40 p-2 md:p-4 flex flex-col items-center shrink-0">
        <div className="w-full flex justify-between items-start max-w-6xl">
           ...
           <Button 
             ...
             className="bg-white/50 hover:bg-white/80 rounded-full text-green-900"
           >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md p-6 rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-40 border-t-4 border-green-200">
         <div className="max-w-md mx-auto flex justify-center gap-8 md:gap-16">
            <Tool type="seed" isMuted={isMuted} />
            <Tool type="water" isMuted={isMuted} />
         </div>
