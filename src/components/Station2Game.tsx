...
import { Recycle, Trash2, Waves, Ban, Volume2, VolumeX } from 'lucide-react';
...
// --- Audio Helper ---
// This simple system expects files in the public/sounds/ directory.
// You need to add: pop.mp3, success.mp3, error.mp3, splash.mp3, win.mp3
const playSound = (soundName: string, isMuted: boolean) => {
  if (isMuted) return;
  ...
  if (soundName === 'error') audio.volume = 0.3;
  if (soundName === 'win') audio.volume = 0.6;
...
function DraggableTrash({ item, isRejected, isMuted }: { item: TrashItem, isRejected: boolean, isMuted: boolean }) {
...
function GameContent({ onComplete }: { onComplete: () => void }) {
  ...
  const playVoice = (filename: string) => {
    ...
    audio.volume = 1.0; // Voices loud and clear
    currentVoiceRef.current = audio;
    audio.play().catch(e => console.log("Voice play failed:", e));
  };

  // Effect to trigger voice when message changes
  useEffect(() => {
    // Map messages to audio files
    let voiceFile = '';
    switch (guideMessage) {
      case "¡El río está contaminado, ayúdanos a limpiarlo!":
        voiceFile = 'voice_intro';
        break;
      case "¡Fantástico! El reciclaje ayuda al planeta. ♻️✨":
        voiceFile = 'voice_success_recycle';
        break;
      case "¡Excelente! Lo orgánico volverá a la tierra. 🍌🌱":
        voiceFile = 'voice_success_organic';
        break;
      case "¡Alto! Eso es basura orgánica, no reciclable. 🚫🍎":
        voiceFile = 'voice_error_organic_in_recycle';
        break;
      case "¡Espera! Eso es material reciclable, no orgánico. 🚫🥤":
        voiceFile = 'voice_error_recycle_in_organic';
        break;
      case "¡Lo lograste! ¡Peces felices! 🐟✨":
        voiceFile = 'voice_complete';
        break;
    }
  ...
  const handleDrop = (id: number, targetBinType: TrashType) => {
    ...
    if (item.type === targetBinType) {
      ...
      // Play splash shortly after error sound
      setTimeout(() => playSound('splash', isMuted), 300);
      ...
      <div className="relative z-40 p-2 md:p-4 flex flex-col items-center shrink-0">
        <div className="w-full flex justify-between items-start max-w-6xl">
           ...
           <Button 
             ...
             className="bg-white/50 hover:bg-white/80 rounded-full text-blue-900"
           >
